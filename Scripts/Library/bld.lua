...
function ClearBuildingStash(BldAlias, OwnerAlias)
	...
	elseif BuildingGetType(BldAlias) == GL_BUILDING_TYPE_HOSPITAL then
		-- TODO remove all items from medicine stash and credit current market value
...
function GetScaffoldOffsets(Proto)
	...
	elseif Proto == 1002 then -- Piratenest
		OffsetX = 750
		OffsetZ = -1900
	elseif Proto == 155 then -- Glazory1
		OffsetX = -100
		OffsetZ = 100
	elseif Proto == 156 then -- Glazory2
		OffsetX = -200
		OffsetZ = 200
	elseif Proto == 157 then -- Glazory3
		OffsetX = -200
		OffsetZ = 200
	end
...
function SetupAI(BldAlias)
	...
	elseif BldType == GL_BUILDING_TYPE_FRUITFARM then
		Items = {	"Salat", "SugarBeet", "Fruit", "Honey"
					}
		ItemsNum = 4
		MarketStock = { 30, 70, 60, 60 }
		LocalStock = { -1, -1, -1, -1 }
...
--[[
---- returns the list of items (by name) that can be sold at this workshop 
function GetItemsForSale(BldAlias)
	local Count = 0
	local Items = {}
	
	if BuildingGetClass(BldAlias) ~= GL_BUILDING_CLASS_WORKSHOP then
		return Count, Items
	end
	
	local Type = BuildingGetType(BldAlias)
	local Lvl = BuildingGetLevel(BldAlias)
	-- XXX artisan buildings
	if BuildingGetType == GL_BUILDING_TYPE_SMITHY then
		local BaseItems = {"Nails", "Tool", "Dagger", "SilverRing", "IronBracelet", "BeltOfMetaphysic", "Platemail", "Shortsword", "IronCap", "Diamond", "FullHelmet"}
		local BaseCount = 11
		for i=1, BaseCount do
			if BuildingHasUpgrade(BaseItems[i]) then
				Count = Count + 1
				Items[Count] = BaseItems[i]
			end 
		end
	elseif BuildingGetType == GL_BUILDING_TYPE_TAILORING then
		if Lvl == 1 then
			Items = {"MoneyBag", "Blanket", "FarmersClothes"}
			Count = 3
		elseif Lvl == 2 then
			Items = {"MoneyBag", "Blanket", "FarmersClothes",
				"LeatherGloves", "CamouflageCloak"}
			Count = 5
		elseif Lvl == 3 then
			Items = {"MoneyBag", "Blanket", "FarmersClothes",
				"LeatherGloves", "CamouflageCloak",
				"NoblesClothes", "GlovesOfDexterity", "LeatherArmor"}
			Count = 8
		end
	elseif BuildingGetType == GL_BUILDING_TYPE_JOINERY then
		if Lvl == 1 then
			Items = {"Torch", "WalkingStick", "BuildMaterial"}
			Count = 3
		elseif Lvl == 2 then
			Items = {"Torch", "WalkingStick", "BuildMaterial",
				"Holzpferd", "Kamm", "Mace", "CartBooster"}
			Count = 7
		elseif Lvl == 3 then
			Items = {"Torch", "WalkingStick", "BuildMaterial",
				"Holzpferd", "Kamm", "Mace", "CartBooster",
				"CrossOfProtection", "RubinStaff", "Axe"}
			Count = 10
		end
	elseif BuildingGetType == GL_BUILDING_TYPE_STONEMASON then
		if Lvl == 1 then
			Items = {"Grindingbrick", "vase"}
			Count = 2
		elseif Lvl == 2 then
			Items = {"Grindingbrick", "vase",
				"Stonerotary", "Blissstone", "Gravestone"}
			Count = 5
		elseif Lvl == 3 then
			Items = {"Grindingbrick", "vase",
				"Stonerotary", "Blissstone", "Gravestone",
	...
	-- XXX patron buildings
	-- XXX scholar buildings
	-- XXX rogue buildings
	end
end
--]]
